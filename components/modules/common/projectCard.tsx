    ...
    <div className="w-full h-full min-h-[240px] bg-[#FAFAFA] shadow-md hover:shadow-lg transition-all duration-300 flex flex-col overflow-hidden rounded-[8.5px]">
      ...
      <div className="px-3 pt-2 pb-4 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-center gap-1 text-gray-600 mb-3">
          ...
          <p className="text-[11px]">{props.location}</p>
        </div>
          ...
          <div className="flex flex-wrap gap-1.5 justify-center">
            {hasSkills ? (
              props.skills.slice(0, 3).map((skill) => (
                <span
                  key={skill}
                  className="text-[#31A7AC] bg-white border border-[#31A7AC] text-[10px] font-medium px-2 py-1 rounded-full whitespace-nowrap"
                >
                  {skill}
                </span>
              ))
