...
export default function ProjectCard(props: ProjectCardType) {
  ...
  return (
    <div className="w-full max-w-[320px] h-[440px] bg-[#FAFAFA] shadow-lg rounded-2xl mx-auto hover:shadow-xl transition-shadow flex flex-col">
      ...
      <div className="flex justify-center items-start -mt-[48px] mx-auto relative z-10">
      ...
      <div className="px-4 pb-4 pt-3 flex flex-col flex-1">
        ...
        <div className="flex justify-center items-center gap-1 text-gray-500 mb-3">
        ...
        <div className="w-full h-[60px] mb-3 flex items-start justify-center">
          <p className="w-full text-gray-600 text-center text-sm line-clamp-3">
            {hasBio ? props.bio : "No bio"}
          </p>
        </div>

        {/* Skills Section - Fixed height */}
        <div className="w-full h-[60px] flex flex-wrap gap-1.5 justify-center content-start">
          {hasSkills ? (
            <>
              {props.skills.slice(0, 3).map((skill) => (
                <span
                  key={skill}
                  className="text-light-green text-xs font-medium px-3 py-1 border border-light-green rounded-full whitespace-nowrap h-fit"
                >
                  {skill}
                </span>
              ))}
              {props.skills.length > 3 && (
                <span className="text-gray-400 text-xs font-medium px-3 py-1 whitespace-nowrap h-fit">
                  +{props.skills.length - 3}
                </span>
              )}
            </>
