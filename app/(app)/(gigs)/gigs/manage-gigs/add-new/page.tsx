...
const buildMonthSummaries = (dateKeys: number[]) => {
  const monthMap = new Map<
    string,
    { label: string; days: number[]; key: string }
  >();
...
export default function AddGigPage() {
  ...
  const [formValues, setFormValues] =
    useState<GigFormValues>(initialFormValues);
  ...
  const monthDateSummaries = useMemo(
    () => buildMonthSummaries(selectedDates),
    [selectedDates],
  );
  const formattedMonthLabel = format(currentMonth, "MMM yyyy");
  const hasActivePreview = useMemo(() => {
    const textFilled = Object.values(formValues).some(
      (value) => value.trim().length > 0,
    );
    const controlsChanged =
      crewCount !== 1 || isTbc || requestQuote || referenceFile || expiryDate;
  ...
  const previewDetailRows = useMemo(
    () => [
      {
        label: "Role",
        value: capitalizeLabel(formValues.role) || "Not provided",
      },
      {
        label: "Type",
        value: capitalizeLabel(formValues.type) || "Not provided",
      },
      ...
      { label: "Request quote", value: requestQuote ? "Yes" : "No" },
      {
        ...
        value: requestQuote
          ? "Requesting quote"
          : formValues.gigRate
            ? `AED ${formValues.gigRate}`
            : "Not provided",
      },
      { label: "Description", value: formValues.description || "Not provided" },
      {
        label: "Qualifying criteria",
        value: formValues.qualifyingCriteria || "Not provided",
      },
      {
        label: "Dates selected",
        value: monthDateSummaries.length
          ? monthDateSummaries
              .map((entry) => `${entry.label} | ${entry.ranges}`)
              .join("; ")
          : "No dates selected",
      },
      {
        label: "Individual dates",
        value: formattedSelectedDates.length
          ? formattedSelectedDates.join(", ")
          : "No dates selected",
      },
      {
        label: "Reference URL",
        value: formValues.referenceUrl || "Not provided",
      },
      {
        label: "Reference file",
        value: referenceFile?.name || "No file uploaded",
      },
      {
        label: "Expiry date",
        value: formValues.expiryDate || "Auto-expire in 3 days",
      },
    ],
    [
      formValues,
  ...
  const referenceLabel = referenceFile
    ? referenceFile.name
    : formValues.referenceUrl;
      ...
      <div className="grid w-full max-w-[1100px] gap-8 lg:grid-cols-[1.2fr_0.8fr]">
        <form onSubmit={handleSubmit} className="w-full mb-10">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-[20px] font-[400] text-[#1D1D1F]">
                Create GIG
              </h1>
            </div>
            <button
          ...
          <section className="mt-8 space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-[30px] sm:pl-5">
              <Label className="text-[18px] font-[400] text-[#1D1D1F]">
                Number of crew
              </Label>
              <div className="flex items-center gap-3">
                <div className="inline-flex items-center rounded-[15px] h-[50px] w-[125px] border border-[#DEDEDE] bg-[#FFFFFF] px-3 py-1">
                  ...
                  <span className="px-4 text-lg font-[400] text-[#1D1D1F]">
                    {crewCount}
                  </span>
                  <button
                    type="button"
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-[24px] p-4">
                <div className="mx-auto sm:mx-0 mb-5 flex w-full max-w-[425px] flex-row items-center justify-between gap-2 rounded-[10px] p-[10px] text-[#1D1D1F] bg-[#ffffff] min-h-[56px]">
                  <span className="text-[24px] font-[400] text-[#FA596E]">
                ...
                <ScrollArea className="mx-auto sm:mx-0 max-h-[260px] w-full max-w-[425px] rounded-b-[10px] bg-[#ffffff]">
                  <div className="grid grid-cols-7 gap-x-0 gap-y-2">
                    {calendarDays.map((day, index) => {
                      ...
                      const dayKey = getDateKey(day);
                      const isSelected =
                        isCurrentMonthDay && selectedDates.includes(dayKey);
                      const columnIndex = index % 7;
                      const atRowStart = columnIndex === 0;
                      const atRowEnd = columnIndex === 6;
                      const prevCell =
                        columnIndex > 0 ? calendarDays[index - 1] : null;
                      const nextCell =
                        columnIndex < 6 ? calendarDays[index + 1] : null;
              ...
              <div className="ml-0 flex items-center gap-3 sm:ml-10">
                <Checkbox
                  id="tbc"
                  checked={isTbc}
                  onCheckedChange={() => setIsTbc((prev) => !prev)}
                  className="h-[27px] w-[27px]"
                />
                <Label
                  htmlFor="tbc"
                  className="text-[18px] font-[400] text-[#1D1D1F]"
                >
                  TBC
                </Label>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-4 md:grid-cols-[1.5fr_1fr]">
                <Input
                  ...
                  onChange={(event) =>
                    handleFieldChange("referenceUrl", event.target.value)
                  }
                  className="h-12 rounded-2xl border-[#646464] bg-[#ffffff] text-[#515151]"
                />
                <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(event) =>
                  handleFieldChange("qualifyingCriteria", event.target.value)
                }
                className="min-h-[120px] rounded-2xl border-[#646464] bg-[#ffffff] text-[#515151]"
              />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col w-full items-start gap-3">
                <div className="flex flex-1 items-center rounded-[15px] w-full border border-[#646464] bg-[#ffffff] text-[#515151]">
                  <div>
                    <span className="px-4 text-sm font-[400] text-[#7B7B7B]">
                      AED
                    </span>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      handleFieldChange("gigRate", event.target.value)
                    }
            ...
            <div className="space-y-1">
              <Label className="text-[18px] font-[400] text-[#444444]">
                GIG expiry date
              </Label>
              <p className="text-[14px] font-[400] text-[#444444]">
                The GIG will expire after 3 days if a date is not selected
              </p>
              <Popover>
          ...
          <div className="mt-8 flex flex-col gap-3 border-t border-[#F0F0F0] pt-6 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-1 justify-end gap-3">
              <Button
                type="button"
                variant="outline"
                className="rounded-[10px] border-[#2AA9A7] text-[#2AA9A7] h-[47px] w-[102px]"
              >
                Publish
              </Button>
              <Button
                type="submit"
                className="rounded-[10px] bg-[#2AA9A7] px-6 text-white h-[47px] w-[102px]"
              >
                Save to draft
              </Button>
            </div>
            ...
            {hasActivePreview ? (
              <>
                ...
                <p className="mt-4 text-lg font-[400] text-[#1D1D1F]">
                  {crewCount} {capitalizeLabel(formValues.role) || "Crew"} for{" "}
                  {capitalizeLabel(formValues.type) || "Gig"}
                </p>
                <div className="mt-3 flex items-center gap-2 text-sm text-[#6F6F6F]">
                  <MapPin className="h-4 w-4" />
                  <span>
                    {formValues.location || "Location details pending"}
                  </span>
                </div>
                <p className="mt-3 text-sm text-[#6F6F6F]">
                  {formValues.description ||
                    "Description of the GIG will be here ..."}
                </p>
                ...
                <div className="space-y-4 text-sm text-[#1D1D1F]">
                  <div className="flex gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-[400] uppercase tracking-wide text-[#8F8F8F]">
                        Selected dates
                      </p>
                      {monthDateSummaries.length ? (
                        <div className="space-y-1">
                          {monthDateSummaries.map((entry, entryIndex) => (
                            <p
                              key={`date-summary-${entryIndex}`}
                              className="text-sm text-[#4F4F4F]"
                            >
                              <span className="font-[400] text-[#1D1D1F]">
                                {entry.label}
                              </span>
                  ...
                  <p className="text-base font-[400]">
                    {requestQuote
                      ? "Requesting quote"
                      : `AED ${formValues.gigRate || "0"}`}
                  </p>
                  <p className="text-sm font-[400] text-[#1D1D1F]">
                    Qualifying criteria:
                    <span className="pl-1 font-normal text-[#6F6F6F]">
                      {formValues.qualifyingCriteria ||
                        "This is where the qualifying criteria value comes"}
                    </span>
                  </p>
                  {referenceLabel && (
                ...
                <p className="mt-4 text-xs text-[#8F8F8F]">
                  Posted on {format(new Date(), "d MMM, yyyy")}
                </p>
