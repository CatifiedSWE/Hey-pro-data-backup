...
const buildTimelineEntries = (
  windows: (typeof gigsData)[number]["dateWindows"],
...
export function AvailabilityTab({ selectedGigIds }: AvailabilityTabProps) {
  const selectedGigs = gigsData.filter((gig) =>
    ...
    return (
      <Card className="bg-transparent border-none">
        <CardHeader>
          ...
          <CardDescription>
            Choose at least one gig in the Gigs tab to inspect talent
            availability across requested dates.
          </CardDescription>
        </CardHeader>
      </Card>
    );
    ...
    <div className="space-y-8">
      {selectedGigs.map((gig) => {
        ...
        const monthBadges = Array.from(
          new Set(timeline.map((entry) => entry.monthLabel)),
        );
        return (
            ...
            <header className="space-y-3 overflow-x-auto ">
              ...
              <div className="flex flex-row items-center gap-4 w-[950px] rounded-2xl border border-[#EFEFEF] bg-white px-4 py-3 text-sm text-gray-900">
                {gig.dateWindows.map((window, windowIndex) => {
                  ...
                  return (
                    <div
                      key={`${gig.id}-window-${windowIndex}`}
                      className="flex items-center gap-3"
                    >
                      <span className="text-base font-semibold text-[#3B3B3B]">
                        {year}
                      </span>
                      <span className="rounded-full bg-[#FA6E80] px-4 py-1 text-sm font-medium text-white">
                        {month}
                      </span>
              ...
              <p className="text-sm text-gray-600">
                Showing availability for every requested day; days without data
                default to N/A.
              </p>
            </header>
                    ...
                    <tr key={`${gig.id}-availability-${person.id}`}>
                      <td className="sticky left-0 flex w-[160px] items-center justify-center gap-2 bg-white px-4 py-2">
                        ...
                        <div>
                          ...
                          {person.credits ? (
                            <p className="text-xs text-[#31A7AC]">
                              Credits added
                            </p>
                          ) : (
                            <p className="text-xs text-[#31A7AC]">
                      ...
                      {timeline.map((entry) => {
                        ...
                        if (state === "na") {
                          return (
                            <td
                              key={entry.key}
                              className="border h-[41px] w-[38px] px-2 py-1 text-center text-xs text-gray-400"
                            >
                              N/A
                            </td>
                          );
                        ...
                        if (state === "hold") {
                          return (
                            <td
                              key={entry.key}
                              className="border h-[41px] w-[38px] bg-[#6A89BE] px-2 py-1 text-center"
                            >
                        ...
                        return (
                          <td
                            key={entry.key}
                            className="border h-[41px] w-[38px] bg-[#FCAF45] px-2 py-1 text-center"
                          >
