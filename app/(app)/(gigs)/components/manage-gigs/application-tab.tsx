...
type ApplicationTabProps = {
  ...
  actionIndicators: Record<
    string,
    Partial<Record<"release" | "shortlist" | "confirm", boolean>>
  >;
  onActionChange: (
    rowKey: string,
    action: "release" | "shortlist" | "confirm",
  ) => void;
};
                ...
                {gig.dateWindows.map((window, index) => (
                  <span key={`${gig.id}-window-${index}`} className="">
                    <span className="font-[500] text-[14px]">
                      ...
                      <span className="text-[#FA6E80] text-[14px]">
                        {" "}
                    ...
                    {window.range}
                    {index < gig.dateWindows.length - 1 && (
                      <span className="mx-1">·</span>
                    )}
                  </span>
                ))}
            ...
            <table className="min-w-[1057px] border-separate border-spacing-x-[2px] border-spacing-y-0 text-sm">
              <thead className="bg-[#FFFFFF] border text-left h-[55px]">
                <tr className="space-x-1">
                  <th className="border-1 border-[#DEDEDE] px-4 py-3 font-[500] text-[#000000]">
                    Name
                  </th>
                  <th className="border-1 border-[#DEDEDE] px-4 py-3 font-[500] text-[#000000]">
                    City
                ...
                {sampleApplicants.map((person) => {
                  ...
                  return (
                    <tr key={rowKey} className="text-gray-800">
                      ...
                      <td className="border border-[#DEDEDE] w-[100px] px-4 py-3 text-[#27B4BC]">
                        {person.city}
                      </td>
                      <td className="border border-[#DEDEDE] w-[204px] px-4 py-3">
                        {person.skills.join(" | ")}
                      </td>
                      <td className="border border-[#DEDEDE] w-[105px] px-4 py-3 text-[#27B4BC]">
                        {person.credits}
                      </td>
