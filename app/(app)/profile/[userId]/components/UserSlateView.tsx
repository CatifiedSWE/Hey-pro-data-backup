...
import { likePost, unlikePost, savePost, unsavePost, sharePost, SlatePost } from "@/lib/api/slate";
import { useInView } from 'react-intersection-observer';
import axios from "axios";
import CommentsModal from "@/components/modules/slate/CommentsModal";

interface UserSlateViewProps {
  userId: string;
  userName: string;
}

export default function UserSlateView({ userId, userName }: UserSlateViewProps) {
  ...
  const loadPosts = async (pageNum: number = 1) => {
    try {
      ...
      const response = await axios.get('/api/slate', {
        params: {
          page: pageNum,
          limit: 20,
          sort: 'latest',
      ...
      if (response.data.success) {
        const userPosts = response.data.data.posts.filter((post: SlatePost) => post.author.id === userId);
        ...
        setHasMore(response.data.data.pagination.hasMore && userPosts.length > 0);
        setPage(pageNum);
      }
    } catch (error: any) {
  ...
  return (
    <>
      ...
      <CommentsModal
        ...
        onCommentAdded={() => setShowComments(false)}
      />
    </>
  );
}
